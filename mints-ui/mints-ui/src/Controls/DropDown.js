...
export default function DropDown (props) {
	...
	return (
		<FormControl variant='outlined' style={{ margin: 5, padding: 0 }} fullWidth>
			<InputLabel shrink={true} ref={inputLabel} style={{backgroundColor: '#eee', paddingLeft: 5, paddingRight: 5}}>
				{props.label}
			</InputLabel>
			<Select
			...
			>
				...
				{items.map((item, index) => (
					<option style={{ padding: '7px 12px', cursor: 'pointer' }} key={index} value={item}>
						{ props.valueField ? item[props.valueField] : item}
					</option>
				))}
				{props.new && (
